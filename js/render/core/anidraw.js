...
this.keydown = e => {
   ...
   switch (e.code) {
   case 'AltLeft'    : case 'AltRight'    : this.mousedown({x:eventUntransformX(x),
                                                            y:eventUntransformY(y)}); return;
   case 'MetaLeft'   : case 'MetaRight'   : isMeta    = true; return;
   case 'ShiftLeft'  : case 'ShiftRight'  : isShift   = true; e_x = x; e_y = y; return;
   case 'ControlLeft': case 'ControlRight': isControl = true; return;
   }
}
...
this.keyup = e => {
   if (isCodeText) {
      switch (e.keyCode) {
      case  8: codeText = codeText.substring(0, codeText.length-1); return;
      case 32: codeText += ' '; return;
      }
      ...
      codeText += String.fromCharCode(e.keyCode + (isShift ? 0 : 32));
      return;
   }
   ...
   case 't': if (isControl) isCodeText = ! isCodeText; break;
...
let display = () => {
   ...
   if (isCodeText) {
      ...
      for (let n = 0 ; n < s.length ; n++)
         context.fillText(s[n], 20, 100 + 30 * n);
      return;
   }
