export function Anidraw(canvas) {
   ...
   let isPathIntersection = (nA,nB) => {
      ...
      let dA = timeOffset(nA);
      let dB = timeOffset(nB);
   ...
   this.textLabel = (context,str,p,h,pad,font,bg,fg) => {
      if (h    === undefined) h    = 30;
      if (pad  === undefined) pad  = 5;
      if (font === undefined) font = 'courier';
      if (bg   === undefined) bg   = 'white';
      if (fg   === undefined) fg   = 'black';
   
      context.font = h + 'px ' + font;
      context.fillStyle = bg;
      context.fillRect(p[0]-pad, p[1]-h*7/10-pad, context.measureText(str).width+2*pad, h+2*pad);
      context.fillStyle = fg;
      context.fillText(str, p[0], p[1]);
   }
   ...
   this.keydown = e => {
      if (isCodeText) {
         switch (e.key) {
         case 'Control': isControl = true; return;
         case 'Meta'   : isMeta    = true; return;
         case 'Shift'  : isShift   = true; return;
         }
      }
   
      switch (e.key) {
      case 'Alt'    : this.mousedown({x:eventUntransformX(x),
                                      y:eventUntransformY(y)}); return;
      case 'Control': isControl = true; return;
      case 'Meta'   : isMeta    = true; return;
      case 'Shift'  : isShift   = true; e_x = x; e_y = y; return;
      }
   }
   ...
   this.keyup = e => {
      ...
      case 'MetaLeft'   : case 'MetaRight'   : isMeta    = false; return;
      case 'ShiftLeft'  : case 'ShiftRight'  : isShift   = false; return;
      case 'ControlLeft': case 'ControlRight': isControl = false; return;
   ...
   let display = () => {
      ...
      if (isCodeText) {
         ...
         for (let n = 0 ; n < s.length ; n++) {
            ...
            context.fillRect(20, y-22, s[n].length * 18, 30);
            context.fillStyle = 'black';
            context.fillText(s[n], 20, y);
       if (codeTextIndex >= offset-1 && codeTextIndex < offset + s[n].length) {
          codeTextIndexWithinLine = codeTextIndex - offset;
               let x = 20 + (codeTextIndex-offset+1) * 18;
               context.fillStyle = 'blue';
               context.fillRect(x-7, y-22- 1, 15,  2);
               context.fillRect(x-1, y-22   ,  2, 30);
               context.fillRect(x-7, y-22+30, 15,  2);
               context.fillStyle = 'black';
            }
       offset += s[n].length + 1;
         }
      ...
      let drawPath = (path, timeOffset) => {
         let textObects = [];
         context.beginPath();
         for (let i = 0 ; i < path.length ; i++) {
            let p = path[i];
            if (p.type == 'text')
          textObects.push({x:timeOffset.x+p.x, y:timeOffset.y+p.y, text:p.text});
            else if (path[i].type == 'move')
               context.moveTo(timeOffset.x+p.x, timeOffset.y+p.y);
            else
               context.lineTo(timeOffset.x+p.x, timeOffset.y+p.y);
         }
         context.stroke();
      ...
      for (let n = 0 ; n < shapes.length ; n++) {
         ...
         if (shapes[n].timeBegin <= timeNow && shapes[n].timeEnd > timeNow)
            drawPath(shapes[n].draw, timeOffset(n));
      }
