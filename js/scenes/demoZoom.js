...
export const init = async model => {
    ...
    let text = model.add('label').color(0,1,1).flag('uTransparentTexture');
    let diagram = model.add('cube').texture('media/textures/truncated_icosahedron.jpg')
                                   .flag('uTransparentTexture');
    model.animate(() => {
       model.hud();
       ...
       if (state > 4) diagram.identity().move(-.4,.3,-.2).turnZ(.3 * Math.sin(5 * c)).scale(.25,.25,.0001);
       if (state > 3) b += model.deltaTime;
       if (state > 2) text.identity().move(.4,.1 * Math.sin(10 * b) + .3,0).scale(.05);
       if (state > 1) a += model.deltaTime;
       if (state > 0) ball.identity().move(.3 * Math.sin(a),-.3,0).scale(.2,.2,.15);
    });
 }
